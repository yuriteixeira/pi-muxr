...
export const TMUX_PANES_FORMAT =
  "#{session_name}\t#{window_index}\t#{window_name}\t#{pane_id}\t#{pane_active}\t#{pane_current_command}\t#{pane_title}\t#{pane_pid}";
...
export function parseTmuxPaneLine(line: string): TmuxPane | null {
  const [sessionName, windowIndex, windowName, paneId, paneActive, currentCommand, paneTitle, panePid] =
    line.split("\t");
...
export function listTmuxPanes(): TmuxPane[] {
  try {
    const output = execFileSync("tmux", ["list-panes", "-a", "-F", TMUX_PANES_FORMAT], {
      encoding: "utf8",
